...
def translate(input_text):
    ...
    with torch.no_grad():
        translated_ids = model.generate(input_ids=inputs['input_ids'], attention_mask=inputs['attention_mask'], num_beams=4, length_penalty=length_penalty)
...
def load_utilities(model_dir):
    # Giving you this as an option as well so that they don't have to be loaded again on every translation
    model = AutoModelForSeq2SeqLM.from_pretrained(model_dir)
    tokenizer = AutoTokenizer.from_pretrained('Helsinki-NLP/opus-mt-en-nl')
